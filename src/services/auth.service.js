...
const authService = {
  ...
  async login({ email, password }) {
    ...
    // 2. Jika user tidak ada — gunakan pesan generik untuk keamanan
    if (!user) {
  ...
  async refresh(tokenString) {
    ...
    if (storedToken && storedToken.isRevoked) {
      // Revoke SEMUA token milik user ini sebagai tindakan pencegahan hacker
    ...
    // Fetch detail user secara internal via client prisma untuk data payload baru
    const userDetail = await prisma.user.findUnique({ where: { id: storedToken.userId } });

    // 6. Buat token baru
    const newAccessToken = signAccessToken({ userId: storedToken.userId, email: userDetail.email });
